...
# seismo pair mappings based on seismo.yaml:
# Pair 1: X1train.npz
...
if pair_id == 1:
    arr, _ = ctf4science.data_module.load_dataset('seismo', pair_id, transpose=False)
    arr = arr[0]
    load_string = 'X1train'
elif pair_id in [2, 3]:
    arr, _ = ctf4science.data_module.load_dataset('seismo', pair_id, transpose=False)
...
np.savez('pairid' + str(pair_id) + 'seismo.npz', data_mat=pred_arr)
